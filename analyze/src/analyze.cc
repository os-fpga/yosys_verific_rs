...
#include <limits>
#ifndef PATH_MAX
//PATH_MAX is defined as MAX_PATH in Windows
        ...
        while (std::getline(in, line)) {
            ...
            while (argidx < size) { 
                        ...
                        if (args[argidx].find("*.") != std::string::npos) {
                            ...
                            for (auto const& dir_entry : fs::directory_iterator{dir}) 
                                if (dir_entry.path().extension() == ".v" || dir_entry.path().extension() == ".sv") {
                                    /****************************************************************
                                    generic_string().c_str() instad of c_str() because c_str()
                                    return type is a synonym for wchar_t on Windows or char on POSIX
                                    *****************************************************************/
                                    file_names.Insert(Strings::save(dir_entry.path().generic_string().c_str()));
                                }
                            argidx++;
                        } else {
                ...
                if (args[argidx] == "-vhdl87") {
                    ...
                    vhdl_file::SetDefaultLibraryPath((vhdl_packages / "vdbs_1987").generic_string().c_str());
                } else if (args[argidx] == "-vhdl93") {
                    analysis_mode = vhdl_file::VHDL_93;
                    vhdl_file::SetDefaultLibraryPath((vhdl_packages / "vdbs_1993").generic_string().c_str());
                    ...
                    vhdl_file::SetDefaultLibraryPath((vhdl_packages / "vdbs_2008").generic_string().c_str());
                } else {
                    continue;
                }
                        ...
                        if (args[argidx].find("*.") != std::string::npos) {
                            ...
                            for (auto const& dir_entry : fs::directory_iterator{dir}) 
                                if (dir_entry.path().extension() == ".vhd")
                                    if (!vhdl_file::Analyze(dir_entry.path().generic_string().c_str(), work.c_str(), analysis_mode)) {
